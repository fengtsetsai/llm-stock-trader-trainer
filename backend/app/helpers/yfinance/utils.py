...
class YFinanceService:
    ...
    def get_stock_data(
        ...
        try:
            if period:
                ...
                end_date_inclusive = (pd.to_datetime(end_date) + pd.DateOffset(days=1)).strftime(
                    "%Y-%m-%d"
                )
    ...
    def get_analyst_recommendations(symbol: str) -> AnalystRecommendation:
        ...
        try:
            ...
            majority_recommendations = latest_rec[latest_rec == max_votes].index.tolist()

            return AnalystRecommendation(
                recommendation=majority_recommendations[0] if majority_recommendations else None,
